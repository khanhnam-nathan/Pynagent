...
use pyneat_rs::{JavaScriptScanner, TypeScriptScanner, GoScanner, JavaScanner, CSharpScanner, PhpScanner, RubyScanner, RustScanner};
use pyneat_rs::LanguageScanner;
use pyneat_rs::scanner::taint::engine::TaintEngine;
use pyneat_rs::scanner::taint::rules::all_taint_rules;
use pyneat_rs::scanner::multilang::parse_ln_ast;
use pyneat_rs::scanner::ln_ast::LnAst;
use pyneat_rs::scanner::supplychain::lock_parser::{parse_package_lock, parse_cargo_lock, check_go_sum, check_requirements_hash_mode};
use pyneat_rs::scanner::supplychain::license::{detect_from_license_file, generate_spdx_from_packages, generate_cyclonedx_from_packages};
use pyneat_rs::scanner::supplychain::{LockPackage, IntegrityStatus};
use std::sync::Arc;
use rayon::prelude::*;
...
fn test_sarif_output_generation() {
    use pyneat_rs::sarif::writer::SarifBuilder;

    let mut builder = SarifBuilder::new("PyNEAT", "1.0.0", "https://github.com/pyneat/pyneat");

    let location = pyneat_rs::sarif::SarifLocation::new(
    ...
    let result = pyneat_rs::sarif::SarifResult::new(
        "SEC-001",
        "critical",
    ...
    assert_eq!(sarif_json["runs"][0]["tool"]["driver"]["name"], "PyNEAT");
...
fn test_sarif_ai_fix_properties() {
    ...
    let mut result = pyneat_rs::sarif::SarifResult::new(
        "SEC-002",
        "critical",
...
fn test_llm_analysis_result_deserialization() {
    ...
    let result: pyneat_rs::ai_analysis::LlmAnalysisResult = serde_json::from_str(json).expect("Should deserialize");
...
fn test_ai_fix_conversion() {
    ...
    let result: pyneat_rs::ai_analysis::LlmAnalysisResult = serde_json::from_str(json).expect("Should deserialize");
    let fix: Option<pyneat_rs::ai_analysis::AiFix> = result.into();
    assert!(fix.is_some());

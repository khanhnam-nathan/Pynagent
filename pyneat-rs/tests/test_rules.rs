...
fn test_parse_npm_package_lock_v3() {
  ...
  "packages": {
    ...
    let packages = pyneat_rs::scanner::supplychain::lock_parser::parse_package_lock(lock_content);
...
"#;
    ...
    let packages = pyneat_rs::scanner::supplychain::lock_parser::parse_cargo_lock(cargo_lock);
...
"#;
    ...
    let results = pyneat_rs::scanner::supplychain::lock_parser::check_go_sum(go_sum);
...
"#;
    ...
    let results = pyneat_rs::scanner::supplychain::lock_parser::check_requirements_hash_mode(req_txt);
    // Packages with hashes are fine
    let missing = results.iter().find(|r| r.status == pyneat_rs::scanner::supplychain::lock_parser::IntegrityStatus::MissingHash);
...
fn test_detect_license_mit() {
    ...
    let licenses = pyneat_rs::scanner::supplychain::license::detect_from_license_file(content);
...
fn test_generate_spdx_sbom_structure() {
  ...
  "packages": {
    ...
    let packages = pyneat_rs::scanner::supplychain::lock_parser::parse_package_lock(lock_content).unwrap();
    let sbom = pyneat_rs::scanner::supplychain::license::generate_spdx_from_packages(&packages, "test-project");
...
fn test_generate_cyclonedx_sbom_structure() {
  ...
  "packages": {
    ...
    let packages = pyneat_rs::scanner::supplychain::lock_parser::parse_package_lock(lock_content).unwrap();
    let sbom = pyneat_rs::scanner::supplychain::license::generate_cyclonedx_from_packages(&packages, "test-project");

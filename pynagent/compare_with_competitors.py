#!/usr/bin/env python3
"""
Pynagent Competitor Comparison Script

Compare Pynagent with other code analysis tools:
- ruff: Fast Python linter
- bandit: Security linter for Python
- pylint: Full Python code analyzer
- pyright: Static type checker

Run: python Pynagent-rs/compare_with_competitors.py
"""
...
def measure_Pynagent(files: List[Path]) -> ToolResult:
    """Measure Pynagent performance."""
    from Pynagent.cli import _build_engine
    from Pynagent.core.engine import clear_module_cache
...
def generate_markdown_table(results: ComparisonResult) -> str:
    ...
    for result in [results.Pynagent, results.ruff, results.bandit, results.pylint, results.pyright]:
        if result is None:
            continue
    ...
    # Calculate speedup
    Pynagent_time = results.Pynagent.avg_time_ms
    for result in [results.ruff, results.bandit]:
        if result and result.avg_time_ms > 0:
            speedup = result.avg_time_ms / Pynagent_time
            lines.append(f"- {result.tool_name} is {speedup:.1f}x slower than Pynagent")
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare Pynagent with competitor tools",
    )
    parser.add_argument(
        "--path", "-p", nargs="+", default=["Pynagent"],
        help="Paths to scan (default: Pynagent)",
    )
    parser.add_argument(
    ...
    print("=" * 60)
    print("Pynagent Competitor Comparison")
    print("=" * 60)
    ...
    print("  - Pynagent...", end=" ", flush=True)
    Pynagent_result = measure_Pynagent(files)
    print(f"done ({Pynagent_result.avg_time_ms:.2f}ms avg)")
    ...
    print("-" * 50)
    print(f"{'Pynagent':<15} {Pynagent_result.avg_time_ms:>10.2f}ms {Pynagent_result.issues_found:>10} {Pynagent_result.files_processed:>8}")
    ...
    Pynagent_time = Pynagent_result.avg_time_ms
    for result, name in [(ruff_result, "ruff"), (bandit_result, "bandit")]:
        if result and result.avg_time_ms > 0 and Pynagent_time > 0:
            speedup = result.avg_time_ms / Pynagent_time
            if speedup > 1:
                print(f"  Pynagent is {speedup:.1f}x FASTER than {name}")
            else:
                print(f"  Pynagent is {1/speedup:.1f}x SLOWER than {name}")
    ...
    if args.output:
        results_dict = {
            ...
            "Pynagent": {
                "tool": Pynagent_result.tool_name,
                "avg_time_ms": Pynagent_result.avg_time_ms,
                "issues_found": Pynagent_result.issues_found,
                "files_processed": Pynagent_result.files_processed,
                "success": Pynagent_result.success,
            },
        }
        if ruff_result:
            results_dict["ruff"] = {

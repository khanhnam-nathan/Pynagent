"""Configuration management for Pynagent.
...
Supports:
- pyproject.toml section [tool.Pynagent]
- .Pynagent.toml file
- Pynagent.toml file
- Environment variables: Pynagent_*
"""
...
class PynagentConfig:
    """Configuration manager for Pynagent."""
    ...
    def _load_config(self, config_path: Optional[Path] = None) -> None:
        ...
        # Priority: explicit path > .Pynagent.toml > Pynagent.toml > pyproject.toml > env
        ...
        # 2. .Pynagent.toml in current directory
        Pynagent_toml = Path.cwd() / '.Pynagent.toml'
        if Pynagent_toml.exists():
            self._load_from_file(Pynagent_toml)
            return

        # 3. Pynagent.toml in current directory
        Pynagent_config = Path.cwd() / 'Pynagent.toml'
        if Pynagent_config.exists():
            self._load_from_file(Pynagent_config)
            return

        # 4. pyproject.toml section [tool.Pynagent]
    ...
    def _load_from_pyproject(self, path: Path) -> None:
        """Load config from pyproject.toml [tool.Pynagent] section."""
        try:
            if sys.version_info >= (3, 11):
                import tomllib
                with open(path, 'rb') as f:
                    data = tomllib.load(f)
                    Pynagent_config = data.get('tool', {}).get('Pynagent', {})
                    self._apply_dict(Pynagent_config)
            else:
...
_config: Optional[PynagentConfig] = None


def get_config() -> PynagentConfig:
    """Get global configuration instance."""
    global _config
    if _config is None:
        _config = PynagentConfig()
    return _config


def reload_config(config_path: Optional[Path] = None) -> PynagentConfig:
    """Reload configuration from file."""
    global _config
    _config = PynagentConfig(config_path)
    return _config
...
class IgnoreManager:
    ...
    Ignores are stored in pyproject.toml [tool.Pynagent] section:
    ...
    def _find_config(self) -> Optional[Path]:
        ...
        search.append(P.cwd() / ".Pynagent.toml")
        search.append(P.cwd() / "Pynagent.toml")
        for p in search:
            if p.exists():
                return p
    ...
    def _save(self) -> None:
        ...
        try:
            ...
            if "tool" not in data:
                data["tool"] = {}
            if "Pynagent" not in data["tool"]:
                data["tool"]["Pynagent"] = {}

            # Write disabled rules
            data["tool"]["Pynagent"]["disabled_rules"] = self._global_disabled
            ...
            data["tool"]["Pynagent"]["ignore_rules"] = ignore_list
    ...
    def _write_toml(self, f, data: Dict[str, Any], indent: int = 0) -> None:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                f.write(f"{prefix}[tool.Pynagent.{key}]\n" if indent == 0 and key == "Pynagent" else f"{prefix}[{key}]\n")
                self._write_toml(f, value, indent + 1)
            elif isinstance(value, list):
                f.write(f"{prefix}[[tool.Pynagent.ignore_rules]]\n" if key == "ignore_rules" else f"{prefix}{key} = []\n")
                for item in value:

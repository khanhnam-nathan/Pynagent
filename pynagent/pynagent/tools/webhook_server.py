"""Webhook server for Pynagent - receive scan results and send real-time notifications.
...
class WebhookServer:
    """HTTP server for receiving and processing Pynagent webhooks.
    ...
    def __init__(self, host: str = "0.0.0.0", port: int = 8080,
        ...
        self.host = host
        self.port = port
        self.api_key = api_key or os.environ.get("Pynagent_WEBHOOK_API_KEY", "")
        self.secret_key = secret_key or os.environ.get("Pynagent_WEBHOOK_SECRET", "")
    ...
    def _send_slack(self, channel: NotificationChannel, event: WebhookEvent) -> None:
        ...
        blocks = [
            {
                ...
                "text": {
                    "type": "plain_text",
                    "text": f"Pynagent Scan: {event.event_type}",
                }
            },
            {
                "type": "section",
        ...
        blocks.append({
            "type": "context",
            "elements": [
                {"type": "mrkdwn", "text": f"Sent via Pynagent Webhook | {datetime.now().isoformat()}"}
            ]
        })
    ...
    def run(self) -> None:
        ...
        """
        print(f"Pynagent Webhook Server starting on {self.host}:{self.port}")
    ...
    def _run_server(self) -> None:
        ...
        class Handler(http.server.BaseHTTPRequestHandler):
            def do_POST(self):
                ...
                # Verify signature
                signature = self.headers.get('X-Pynagent-Signature', '')
            ...
            def _handle_metrics(self):
                ...
                self.end_headers()
                metrics = [
                    f"Pynagent_webhook_events_total 0",
                    f"Pynagent_webhook_channels_total {len(server.channels)}",
                ]
                self.wfile.write('\n'.join(metrics).encode())

"""MCP Server for Pynagent — STDIO JSON-RPC 2.0.

This module implements the Model Context Protocol over stdio, allowing
Cursor and other MCP-compatible editors to invoke Pynagent scans as tools.

Run as:
    python -m Pynagent.tools.mcp_server
    Pynagent mcp
...
# Pynagent core imports
from Pynagent.core.engine import RuleEngine
from Pynagent.core.types import CodeFile, AgentMarker, TransformationResult
from Pynagent.core import manifest_schema
from Pynagent.core.manifest_schema import PynagentManifest, SeveritySummary, load_manifest_or_fail, diff_manifests, format_diff
...
TOOLS: List[Dict[str, Any]] = [
    ...
    {
        "name": "Pynagent_aggregate",
        "description": "Merge multiple markers into a Pynagent project manifest (YAML), "
    ...
    {
        "name": "Pynagent_compare",
        "description": "Compare two Pynagent manifest files and return a diff report "
    ...
    },
    {
        ...
        "description": "List all available Pynagent rules with their names, descriptions, "
    ...
    if target.auto_fix_before and target.auto_fix_after:
        ...
        if dry_run:
            ...
            return (
                ...
                f"Note: For safety, Pynagent_auto_fix performs dry-run by default.\n"
                f"Apply the fix manually or use the CLI: Pynagent fix <file>"
            )

    return (
        f"Auto-fix for {marker_id} is available but the fix diff was not pre-computed.\n"
        f"Use the CLI to apply: Pynagent fix --marker {marker_id} <file>"
    )
...
def _handle_lint_prompt(code: str, language: str = "python") -> str:
    ...
    if ai_markers:
        output.append(f"\nPynagent detected {len(ai_markers)} issue(s):")
        for m in ai_markers:
...
def _dispatch_method(method: str, params: Dict[str, Any]) -> Tuple[Any, Optional[int], Optional[str]]:
    ...
    if method == "initialize":
        ...
        try:
            ...
            elif tool_name == "Pynagent_scan_file":
            ...
            elif tool_name == "Pynagent_explain":
                rule_id = arguments.get("rule_id")
                issue_type = arguments.get("issue_type")
                return _handle_explain(rule_id, issue_type), None, None

            elif tool_name == "Pynagent_auto_fix":
            ...
            elif tool_name == "Pynagent_aggregate":
            ...
            elif tool_name == "Pynagent_compare":
            ...
            elif tool_name == "Pynagent_lint_prompt":
                code = arguments.get("code", "")
                language = arguments.get("language", "python")
                if not code:
                    return None, ERROR_INVALID_PARAMS, "Missing required parameter: code"
            ...
            elif tool_name == "Pynagent_list_rules":

...
if TYPE_CHECKING:
    from Pynagent.tools.github_fuzz import FuzzConfig, FuzzResult
...
class FuzzLogger:
    ...
    def __init__(self, output_dir: str = "Pynagent_fuzz_results"):
    ...
    def export_json(self) -> Path:
        ...
        filename = f"Pynagent_FUZZ_REPORT_{self._ts_str}.json"
    ...
    def export_txt(self) -> Path:
        ...
        rule(f"Pynagent FUZZ TEST REPORT — {ts_display}")
        divider()
        lines.append(
        ...
        filename = f"Pynagent_FUZZ_SUMMARY_{self._ts_str}.txt"
        out_path = self.output_dir / filename
        with open(out_path, "w", encoding="utf-8") as f:
            f.write("\n".join(lines))

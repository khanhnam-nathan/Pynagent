"""Pynagent GitHub Fuzz Testing Tool.
...
Liên tục tải các file Python từ GitHub repos nổi tiếng, chạy Pynagent lên
...
class FuzzConfig:
    ...
    max_workers: int = 4
    cache_dir: str = ".Pynagent_fuzz_cache"

    # Output
    output_dir: str = "Pynagent_fuzz_results"
...
def run_fuzz(config: Optional[FuzzConfig] = None) -> List[FuzzResult]:
    ...
    Example:
        from Pynagent.tools.github_fuzz import run_fuzz, FuzzConfig
    ...
    """
    from Pynagent.tools.github_fuzz.fuzz_runner import _run_fuzz
    if config is None:
        config = FuzzConfig()

"""CLI entry point for Pynagent.tools.github_fuzz.
...
Usage:
    python -m Pynagent.tools.github_fuzz
    python -m Pynagent.tools.github_fuzz --repos django/django psf/requests
    python -m Pynagent.tools.github_fuzz --combinations all --max-files 50
    python -m Pynagent.tools.github_fuzz --dry-download
    python -m Pynagent.tools.github_fuzz --resume ./cache.json

Run a fuzz test loop that downloads Python files from GitHub repos,
runs Pynagent with multiple rule combinations, and generates debug reports.
"""
...
# Add Pynagent to path if running as a standalone script
sys.path.insert(0, str(Path(__file__).parent.parent.parent.parent))

from Pynagent.tools.github_fuzz import (
    FuzzConfig,
    run_fuzz,
    DEFAULT_REPOS,
    RULE_COMBINATIONS,
...
from Pynagent.tools.github_fuzz.debug_logger import FuzzLogger
from Pynagent import __version__ as Pynagent_version
...
def _print_header():
    ...
    click.echo(click.style("  Pynagent GitHub Fuzz Tester", fg="cyan", bold=True))
    click.echo(click.style(f"  Version {Pynagent_version}", fg="cyan"))
    click.echo(click.style("=" * 60, fg="cyan"))
    click.echo("")
...
@click.group()
@click.version_option(version="1.0.0", prog_name="Pynagent-fuzz")
@click.option("--color/--no-color", default=True, help="Force colour output")
def cli(color: bool):
    """Pynagent GitHub Fuzz Tester.

    Continuously downloads Python files from popular GitHub repos,
    runs Pynagent with all rule combinations, detects bugs, and writes
...
def run(
    ...
    Downloads Python files from popular GitHub repos and runs Pynagent
    ...
    Examples:
      ...
      python -m Pynagent.tools.github_fuzz run

      # Test all repos with all rule combinations
      python -m Pynagent.tools.github_fuzz run --repos django/django psf/requests --combinations all

      # Use a GitHub token to avoid rate limiting
      python -m Pynagent.tools.github_fuzz run --github-token ghp_xxxx

      # Resume from a previous download
      python -m Pynagent.tools.github_fuzz run --resume ./cache.json
    """
    _print_header()
    ...
    if crashes > 0 or regressions > 0:
        ...
        json_file = Path(output) / f"Pynagent_FUZZ_REPORT_"
        txt_file = Path(output) / f"Pynagent_FUZZ_SUMMARY_"
        click.echo(f"    JSON: {output}/Pynagent_FUZZ_REPORT_*.json")
        click.echo(f"    TXT : {output}/Pynagent_FUZZ_SUMMARY_*.txt")
        sys.exit(1)
    else:
        click.echo("")
...
def test(file_path: str, preset: str) -> None:
    ...
    Examples:
      ...
      python -m Pynagent.tools.github_fuzz test ./myfile.py
      python -m Pynagent.tools.github_fuzz test ./myfile.py --preset all
    """
    _print_header()

    content = Path(file_path).read_text(encoding="utf-8")

    from Pynagent.tools.github_fuzz.fuzz_runner import test_single_file
...
def watch(
    ...
    Examples:
      ...
      python -m Pynagent.tools.github_fuzz watch --repos django/django

      # Watch multiple repos every 30 minutes
      python -m Pynagent.tools.github_fuzz watch --repos django/django psf/requests --interval 1800
    """
...
def ci(
    ...
    Examples:
      ...
      python -m Pynagent.tools.github_fuzz ci --repos django/django

      # Strict mode: fail on Python 2 files too
      python -m Pynagent.tools.github_fuzz ci --repos django/django --fail-on-unsupported
    """
    import json

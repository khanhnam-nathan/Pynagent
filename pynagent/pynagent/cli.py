...
from Pynagent.core.engine import RuleEngine, clear_module_cache
from Pynagent.core.incremental_cache import IncrementalCache
from Pynagent import __version__
from Pynagent.core.types import RuleConfig, AgentMarker
...
from Pynagent.plugins.loader import PluginLoader
...
def _print_logo() -> None:
    """Print minimalist Pynagent logo at the top of terminal output."""
    click.echo("")
    click.echo(click.style(" Pynagent ", fg="white", bg="cyan", bold=True) +
...
def _inject_Pynagent_comments(content: str, markers: list, language: str) -> str:
...
def _load_config(path: Path | None = None) -> dict:
    ...
    if sys.version_info >= (3, 11):
        ...
        with open(path, "rb") as f:
            data = tomllib.load(f)
        return data.get("tool", {}).get("Pynagent", {})
    return {}
...
@click.group()
@click.version_option(version=__version__, prog_name='Pynagent')
...
def cli(color: str):
    """Pynagent - Neat Python AI Code Cleaner."""
    import sys
...
def clean(input_file: str, output: str, lang_opt: str, in_place: bool, verbose: bool,
    ...
    if use_universal:
        try:
            from Pynagent.rules.universal import (
    ...
    if export_manifest:
        manifest_path = input_path.with_suffix('.Pynagent.manifest.yaml')
        try:
    ...
    if annotate:
        markers = getattr(result, 'agent_markers', []) or []
        if markers:
            annotated_content = _inject_Pynagent_comments(
...
@click.option('--incremental', is_flag=True, default=False,
              help='Only process files that changed since last scan (uses .Pynagent-cache/)')
@click.pass_context
def clean_dir(ctx, dir_path: str, pattern: str, in_place: bool, backup: bool,
    ...
    if in_place and backup and not dry_run:
        backup_dir = path / f".Pynagent_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    cache = None  # type: ignore
    if export_manifest:
        manifest_path = Path(dir_path) / '.Pynagent.manifest.yaml'
        try:
            all_markers: list = []
            for r in results:
...
def rules():
    ...
    click.echo("=" * 60)
    click.echo("")
    click.echo("  Pynagent clean file.py                    # Default: safe package")
    click.echo("  Pynagent clean file.py --package conservative  # Adds cleanup rules")
    click.echo("  Pynagent clean file.py --package destructive   # Adds aggressive rules")
    click.echo("")
    click.echo("=" * 60)
...
def _make_dep_finding(df: dict) -> 'DependencyFinding':
    ...
    from Pynagent.core.types import DependencyFinding
...
def check(target, lang_opt, severity, cvss, output, format, fail_on, skip_deps, use_deps, check_cve, check_license, lock_files, rule_filter, exclude, verbose, quiet, use_rust):
    ...
    Examples:
        Pynagent check app.py
        Pynagent check src/ --severity --cvss
        Pynagent check . --lang javascript
        Pynagent check . --fail-on critical --output report.json
        Pynagent check . --format sarif --output results.sarif
    """
    from Pynagent.rules.security import SecurityScannerRule
    from Pynagent.rules.security_registry import SECURITY_RULES_REGISTRY
    from Pynagent.config import IgnoreManager
    from Pynagent.core.engine import RuleEngine
    from Pynagent.core.types import SecurityFinding, SecuritySeverity
    import json
    import time
    ...
    if use_rust is not False:  # None or True = try Rust
        from Pynagent.scanner.rust_scanner import get_scanner
        scanner = get_scanner()
    ...
    if lock_files:
        try:
            from Pynagent_rs import discover_lockfiles as rust_discover
            ...
            click.echo("  Install with: pip install Pynagent[rust]")

    # Full dependency scan
    if not skip_deps and use_deps:
        # Try Rust backend first for deep scanning
        try:
            from Pynagent_rs import scan_dependencies as rust_scan_deps
        ...
        if not dep_findings:
            from Pynagent.tools.security.dependency_scanner import DependencyScanner
...
def explain(rule_id, file, line):
    ...
    Examples:
        ...
        Pynagent explain SEC-014 --file app.py --line 42
    """
    from Pynagent.rules.security_registry import get_security_rule, SECURITY_RULES_REGISTRY
...
def ignore(rule_id, file, line, is_global, reason):
    ...
    Examples:
        Pynagent ignore SEC-001 --file deploy.py --line 42 --reason "already sanitized"
        Pynagent ignore SEC-003 --global --reason "intentional eval in sandbox"
    """
    from Pynagent.config import IgnoreManager
    ...
    if is_global:
        ...
        click.echo(f"     To re-enable: Pynagent ignore --remove {rule_id}")
    elif file and line:
        ...
        click.echo("")
        click.echo("Usage:")
        click.echo("  Per-instance: Pynagent ignore SEC-001 --file app.py --line 42 --reason 'safe'")
        click.echo("  Global:       Pynagent ignore SEC-003 --global --reason 'intentional'")
        return 1
...
def report(target, format, output, use_rust):
    ...
    Supports multiple formats for different tools:
      ...
      - yaml:    Pynagent manifest (full AgentMarker data)
    ...
    Examples:
        Pynagent report . --format sarif --output security.sarif
        Pynagent report . --format yaml --output report.Pynagent.yaml
        Pynagent report . --format junit --output results.xml
    """
    from Pynagent.core.engine import RuleEngine
    from Pynagent.config import IgnoreManager
    ...
    from Pynagent.core.types import AgentMarker, SecuritySeverity, security_finding_to_marker
    import json
    import time
    ...
    for file_path in sorted(files_to_scan):
        ...
        if rust_available:
            ...
            for rf in rust_results:
                ...
                from Pynagent.core.types import security_finding_to_marker
                marker = AgentMarker(
    ...
    if format == 'sarif':
        ...
        junit_xml = export_to_junit_xml(
            markers=all_markers,
            source_file=target_path,
            test_name=f"Pynagent Security Scan ({target_path.name})",
        )
        ...
        html_output = export_to_html_report(
            markers=all_markers,
            title=f"Pynagent Security Report — {target_path.name}",
        )
...
def security_db(update, status, force):
    ...
    Examples:
        Pynagent security-db --status
        Pynagent security-db --update
        Pynagent security-db --update --force
    """
    from Pynagent.tools.security.advisory_db import CVEDatabase, GitHubAdvisoryDB
    import click
    ...
    if update:
        ...
        click.echo("  Run 'Pynagent security-db --status' to verify.")
...
def _handle_menu_choice(choice: str, suggestions: Dict[str, tuple], ctx: click.Context) -> None:
    ...
    if choice in choice_map:
        ...
        click.echo("")
        click.echo(f"  Running: {click.style('Pynagent ' + cmd_name + ' --help', fg='cyan', bold=True)}")
        click.echo("")
        ...
        click.echo("")
        click.echo("  📚 Docs: https://Pynagent.dev/docs")
...
def audit_deps(path, format, output, sbom_format):
    ...
    Examples:
        ...
        Pynagent audit-deps --format sbom --output sbom.json
        Pynagent audit-deps --path requirements.txt --format sarif --output report.sarif
    """
    from Pynagent.tools.vulnerability_scanner import DependencyScanner
...
def sbom_cmd(path, format, output, include_vulns):
    ...
    Examples:
        ...
        Pynagent sbom --format spdx-json --output sbom.json
    """
    from Pynagent.tools.vulnerability_scanner import DependencyScanner
...
def mcp(transport: str, verbose: bool):
    """Start the Pynagent MCP server over stdio (JSON-RPC 2.0).

    This command starts the Model Context Protocol server, allowing Cursor
    and other MCP-compatible editors to invoke Pynagent as a tool.
    ...
    from Pynagent.tools.mcp_server import main
    main()
...
def lsp(stdio, tcp, port, scan_on_save, scan_on_type, debounce_ms, severity, rules):
    """Start Pynagent LSP server for real-time IDE security scanning.
    ...
    Usage:
        Pynagent lsp                  # Start with defaults (stdio mode)
        Pynagent lsp --tcp --port 4444  # TCP mode on port 4444

    For IDE integration, the extension will automatically start the LSP server.
    See .vscode-extension/ for the VS Code extension.

    Examples:
        Pynagent lsp --severity high --debounce-ms 300
        Pynagent lsp --rules SEC-001,SEC-002,SEC-003
    """
    try:
        from Pynagent_rs import run_lsp_server
    except ImportError:
        click.secho("[ERROR] Pynagent Rust backend not installed.", fg="red", bold=True)
        click.echo("  Install with: pip install Pynagent[rust]")
        click.echo("  Or build from source: cd Pynagent-rs && cargo build --release")
        return 1

    click.echo(f"Starting Pynagent LSP server...")
    if stdio:
...
@click.argument('action', type=click.Choice(['run', 'scan', 'status']), default='run')
@click.option('--pr', type=str, help='PR identifier (owner/repo#number)')
@click.option('--owner', type=str, help='Repository owner')
@click.option('--repo', type=str, help='Repository name')
@click.option('--pr-number', type=int, help='Pull request number')
@click.option('--pr-title', type=str, default='', help='Pull request title')
@click.option('--pr-description', type=str, default='', help='Pull request description')
@click.option('--author', type=str, default='unknown', help='PR author')
@click.option('--base-branch', type=str, default='main', help='Base branch')
@click.option('--head-branch', type=str, default='feature', help='Head branch')
@click.option('--head-sha', type=str, default='', help='Head commit SHA')
@click.option('--changed-files', type=str, default='', help='Comma-separated list of changed files')
@click.option('--target', type=str, default='.', help='Directory or file to scan')
@click.option('--languages', type=str, default='python', help='Comma-separated languages to scan')
@click.option('--ai/--no-ai', 'use_ai', default=True, help='Enable AI analysis (AMD Cloud)')
@click.option('--analysis-severity', type=click.Choice(['critical', 'high', 'medium']),
              default='high', help='Minimum severity for AI analysis')
@click.option('--output', '-o', type=click.Path(), help='Output file path')
@click.option('--format', '-f', type=click.Choice(['json', 'text']), default='text',
              help='Output format')
def agent(
    action, pr, owner, repo, pr_number, pr_title, pr_description, author,
    base_branch, head_branch, head_sha, changed_files, target, languages,
    use_ai, analysis_severity, output, format
):
    """Run the Pynagent AI Agent for PR security analysis.

    This command runs the full AI agent pipeline:
    1. Scan changed files with Pynagent (Rust SAST)
    2. Analyze findings with AMD Cloud AI (Qwen/Llama)
    3. Generate human-readable explanations
    4. Prioritize findings by severity and exploitability

    Examples:
        Pynagent agent run --pr owner/repo#42
        Pynagent agent scan --target . --languages python,javascript
        Pynagent agent status
    """
    import asyncio
    import json

    if action == 'status':
        click.echo("Pynagent AI Agent")
        click.echo("=" * 40)
        try:
            from Pynagent.agentic.agent import AMDCloudConfig
            cfg = AMDCloudConfig.from_env()
            if cfg.is_configured():
                click.echo(f"  AMD Cloud: configured ({cfg.model} at {cfg.base_url})")
            else:
                click.echo("  AMD Cloud: not configured (set AMD_API_KEY)")
            from Pynagent.github_app.Pynagent_wrapper import PynagentCLI
            cli = PynagentCLI()
            available, version = cli.check_availability()
            if available:
                click.echo(f"  Pynagent CLI: available ({version})")
            else:
                click.echo(f"  Pynagent CLI: not available")
        except Exception as e:
    ...
    # Build PR context
    from Pynagent.agentic.agent import PRContext, AgentConfig, PynagentAgent
    pr_ctx = PRContext(
    ...
    agent_cfg = AgentConfig(
        enable_analysis=use_ai,
        analysis_min_severity=analysis_severity,
        explain_findings=use_ai,
    )

    async def run_agent():
        from Pynagent.github_app.Pynagent_wrapper import PynagentCLI
        from Pynagent.agentic.agent import AMDCloudConfig, PynagentAgent

        Pynagent_cli = PynagentCLI()
        amd_cfg = AMDCloudConfig.from_env()
        agent = PynagentAgent(Pynagent_cli, amd_cfg, agent_cfg)

        return await agent.run(pr_ctx)

    try:
        click.echo("Pynagent AI Agent")
        click.echo("=" * 40)
        if use_ai:
            click.echo(f"  AI Analysis: enabled (AMD Cloud, min severity: {analysis_severity})")
        else:
            click.echo("  AI Analysis: disabled")
        click.echo(f"  Target: {target}")
        click.echo(f"  Languages: {', '.join(langs) if langs else 'auto-detect'}")
        click.echo(f"  Changed files: {len(files_list) or 'all'}")
        click.echo("")
        ...
        if result.get('status') == 'error':
            return 1
        critical = result.get('scan_result', {}).get('severity_summary', {}).get('critical', 0)
        if critical > 0:
        ...
        click.secho(f"[ERROR] Agent error: {e}", fg="red", bold=True)
        return 1


def _print_agent_result(result):
    """Print agent result in human-readable format."""
    import click

    scan_result = result.get('scan_result', {})
    severity = scan_result.get('severity_summary', {})

    click.echo("")
    click.echo(click.style("  Scan Results  ", bg='cyan', fg='white', bold=True))
    click.echo(f"  Total: {result.get('total_findings', 0)} findings")
    click.echo(f"  Files scanned: {scan_result.get('files_scanned', 0)}")
    click.echo(f"  Duration: {scan_result.get('duration_seconds', 0):.1f}s")
    click.echo("")

    click.echo(f"  {'Severity':<12} {'Count':<8}")
    click.echo(f"  {'-'*20}")
    for sev, label in [('critical','Critical'), ('high','High'), ('medium','Medium'),
                        ('low','Low'), ('info','Info')]:
        count = severity.get(sev, 0)
        color = {'critical': 'red', 'high': 'magenta', 'medium': 'yellow',
                 'low': 'blue', 'info': 'white'}.get(sev, 'white')
        if count > 0:
            click.echo(click.style(f"  {label:<12} {count:<8}", fg=color))
        else:
            click.echo(f"  {label:<12} {count:<8}")

    if result.get('ai_enabled'):
        click.echo("")
        click.echo(click.style("  AI Analysis  ", bg='green', fg='white', bold=True))
        click.echo(f"  True positives:  {result.get('true_positives', 0)}")
        click.echo(f"  False positives: {result.get('false_positives', 0)}")
        click.echo(f"  Needs review:    {result.get('needs_review', 0)}")
    ...
    if findings:
        ...
        for f in findings[:10]:
            ...
            click.echo(click.style(f"  [{sev.upper():>8}]", fg=color) + f" {rule} at {loc}")
            if f.get('ai_analysis'):
                verdict = f['ai_analysis'].get('verdict', '?')
                if verdict == 'FALSE_POSITIVE':
                    click.echo(click.style(f"    -> FALSE POSITIVE", fg='green'))
                elif verdict == 'TRUE_POSITIVE':
                    click.echo(click.style(f"    -> TRUE POSITIVE", fg='red'))
                else:
                    click.echo(click.style(f"    -> {verdict}", fg='yellow'))
...
@cli.command(name='bot-server')
@click.option('--host', default='0.0.0.0', help='Server host')
@click.option('--port', default=8080, help='Server port')
@click.option('--log-level', type=click.Choice(['debug', 'info', 'warning', 'error']),
              default='info', help='Log level')
def bot_server(host, port, log_level):
    """Start the Pynagent Bot Notification Server for GitHub App notifications.

    This FastAPI server receives scan results and forwards them to:
    - Slack webhooks
    - Discord webhooks
    - Email (SMTP)
    - Generic webhooks
    ...
    Endpoints:
      POST /bot/scan-result  - Receive scan results from GitHub App
    ...
    Examples:
        Pynagent bot-server --port 8080
        Pynagent bot-server --host 0.0.0.0 --port 3000 --log-level debug
    """
    import asyncio
    import logging
    ...
    try:
        from Pynagent.tools.bot_notification_server import run_server
        click.echo(f"Starting Pynagent Bot Notification Server on {host}:{port}...")
        run_server(host=host, port=port)
    except ImportError:
        click.secho("Error: FastAPI is required for the bot server.", fg="red")
        click.echo("Install with: pip install fastapi uvicorn aiohttp pydantic")
        return 1
    except KeyboardInterrupt:
        click.echo("\nBot server stopped.")
        return 0
    except Exception as e:
        click.secho(f"[ERROR] Bot server error: {e}", fg="red", bold=True)
        return 1

...
from Pynagent.core.types import AgentMarker, CodeFile, RuleConfig, TransformationResult
...
class InitFileProtectionRule(Rule):
    ...
    def _protect_init_imports(self, content: str, ast_tree=None) -> Tuple[str, List[str]]:
        ...
        # Step 3: Add # Pynagent: protected comment to these imports
        lines = content.split('\n')
        ...
        for ln in sorted(protected_indices, reverse=True):
            ...
            if stripped and not stripped.startswith('# Pynagent: protected'):
                lines[ln] = '# Pynagent: protected\n' + lines[ln]

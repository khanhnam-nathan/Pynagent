...
"""Configuration loader for Pynagent.

Supports loading configuration from:
- .Pynagent.yaml in current directory
- .Pynagent.yml in current directory
- pyproject.toml [tool.Pynagent] section
- Environment variables (Pynagent_*)
- Command-line arguments (passed to load_config)

Example .Pynagent.yaml:
    rules:
      SEC-010:
        enabled: true
        params:
...
class RuleConfigSpec:
    """Per-rule configuration from .Pynagent.yaml.
...
class PynagentConfig:
    """Full Pynagent configuration loaded from .Pynagent.yaml.
    ...
    @classmethod
    def from_yaml(cls, path: Path) -> "PynagentConfig":
        """Load PynagentConfig from a YAML file."""
        try:
...
class ConfigLoader:
    ...
    3. .Pynagent.yaml / .Pynagent.yml
    4. pyproject.toml [tool.Pynagent]
    5. Default values
    """
        ...
        try:
            if sys.version_info >= (3, 11):
                ...
                with open(path, "rb") as f:
                    return tomllib.load(f).get("tool", {}).get("Pynagent", {})
            else:
                import tomli
                with open(path, "rb") as f:
                    return tomli.load(f).get("tool", {}).get("Pynagent", {})
        except ImportError:
            raise
        except Exception:
            return {}
    ...
    def _load_yaml(self) -> Dict[str, Any]:
        """Load configuration from .Pynagent.yaml or .Pynagent.yml."""
        if self.config_path:
        ...
        for name in [".Pynagent.yaml", ".Pynagent.yml"]:
            path = Path.cwd() / name
            if path.exists():
                return self._parse_yaml(path)
    ...
    def _load_env(self) -> Dict[str, Any]:
        ...
        env_mappings = {
            "Pynagent_SEVERITY_THRESHOLD": ("severity_threshold",),
            "Pynagent_RULES_ENABLED": ("rules", "enabled"),
            "Pynagent_RULES_DISABLED": ("rules", "disabled"),
            "Pynagent_SECURITY_FAIL_ON": ("security", "fail_on"),
            "Pynagent_SECURITY_AUTO_FIX": ("security", "auto_fix"),
            "Pynagent_OUTPUT_FORMAT": ("output", "format"),
            "Pynagent_CACHE_ENABLED": ("cache", "enabled"),
        }

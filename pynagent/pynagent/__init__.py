...
A quick-start example:
    ...
    from Pynagent import clean_code, RuleEngine, CodeFile
    ...
    from Pynagent import RuleEngine, DebugCleaner, IsNotNoneRule
...
__all__ = [
    ...
    # Fuzz testing tool
    'github_fuzz',
    # AI Agent (lazy-loaded)
    'PynagentAgent', 'create_agent', 'PRContext',
...
def __getattr__(name: str):
    if name == 'github_fuzz':
        from Pynagent.tools import github_fuzz
        return github_fuzz
    if name == 'PynagentAgent':
        from Pynagent.agentic import PynagentAgent
        return PynagentAgent
    if name == 'create_agent':
        from Pynagent.agentic import create_agent
        return create_agent
    if name == 'PRContext':
        from Pynagent.agentic.agent import PRContext
        return PRContext
    if name == 'get_langchain_tools':
        from Pynagent.agentic.langchain_tools import get_langchain_tools
        return get_langchain_tools
    if name == 'LLMConfig':
        from Pynagent.agentic.agent import LLMConfig
        return LLMConfig
    if name == 'LLMProvider':
        from Pynagent.agentic.agent import LLMProvider
        return LLMProvider
...
def clean_code(
    ...
    This is the simplest way to use Pynagent — just pass a code string.
...
def analyze_code(
    ...
    Example:
        from Pynagent import analyze_code

...
def export_to_sarif(
    ...
    for marker in markers:
        rule_id = f"Pynagent/{marker.rule_id}/{marker.marker_id}"
...
def export_to_sarif_legacy(
    ...
    for marker in markers:
        results.append({
            "ruleId": f"Pynagent/{marker.rule_id}/{marker.marker_id}",
            "level": SEVERITY_TO_SARIF_LEVEL.get(marker.severity, "warning"),
            "message": {
                "text": marker.hint or marker.why or f"Issue: {marker.issue_type}",
            },
...
def export_to_codeclimate(markers: List[AgentMarker], source_file: Path) -> List[Dict[str, Any]]:
    ...
    for m in markers:
        ...
        results.append({
            "type": "ISSUE",
            "check_name": f"Pynagent.{getattr(m, 'rule_id', 'unknown')}",
...
def _severity_to_junit_level(severity: str) -> str:
    """Map Pynagent severity to JUnit testcase status."""
    mapping = {
        "critical": "error",
        "high": "error",
        "medium": "failure",
...
def export_to_html_report(
    markers: List[AgentMarker],
    title: str = "Pynagent Security Report",
...
<div class="header">
  ...
  <p>Scanned with Pynagent &bull; {total} issue(s) found &bull; Generated {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
</div>
<div class="summary">
...
class ManifestExporter:
    ...
    def write(self, source_file: Path) -> Optional[Path]:
        ...
        if key not in self._markers or not self._markers[key]:
            return None
        manifest_path = source_file.with_suffix(source_file.suffix + ".Pynagent.manifest.json")
        manifest_data = {
            "version": "1.0",
...
class MarkerParser:
    ...
    def find_manifest(source_file: Path) -> Optional[Path]:
        ...
        manifest = source_file.with_suffix(source_file.suffix + ".Pynagent.manifest.json")

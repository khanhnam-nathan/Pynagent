...
def _cleanup_tmp_files() -> None:
    """Remove any leftover .Pynagent_tmp files on interpreter exit."""
...
class AtomicWriter:
    ...
    def backup(self, file_path: Path) -> Optional[Path]:
        ...
        backup_path = file_path.with_suffix(file_path.suffix + ".Pynagent.bak")
    ...
    def recover_tmp(self, dir_path: Path) -> int:
        """Remove all leftover .Pynagent_tmp files in a directory tree.

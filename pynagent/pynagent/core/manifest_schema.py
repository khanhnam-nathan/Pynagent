"""Pynagent Manifest V2 schema — lifecycle tracking for AgentMarkers.
...
# --------------------------------------------------------------------------
# PynagentManifest — the core lifecycle document
# --------------------------------------------------------------------------

@dataclass
class PynagentManifest:
    """Pynagent Manifest V2 — tracks scan lifecycle, marker history, and diffs.
    ...
    @classmethod
    def load(cls, path: Path) -> "PynagentManifest":
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "PynagentManifest":
    ...
    def mark_previous(self, previous: "PynagentManifest") -> None:
    ...
    def compute_diff(self, previous: "PynagentManifest") -> Dict[str, Any]:
...
def load_manifest_or_fail(path: Path) -> PynagentManifest:
    """Load a manifest file, raising a clear error if it doesn't exist."""
    if not path.exists():
        raise FileNotFoundError(f"Manifest not found: {path}")
    return PynagentManifest.load(path)


def diff_manifests(current: PynagentManifest, previous: PynagentManifest) -> Dict[str, Any]:

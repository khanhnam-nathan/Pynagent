"""Benchmark script to measure Pynagent performance.
...
Usage:
    python -m Pynagent.benchmark
    python -m Pynagent.benchmark --files Pynagent/rules/*.py
    python -m Pynagent.benchmark --profile
    python -m Pynagent.benchmark --iterations 50

This script benchmarks Pynagent across the codebase itself and provides:
...
def benchmark_all_rules(file_path: Path, iterations: int = 5) -> Dict[str, float]:
    ...
    from Pynagent.core.types import CodeFile
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Benchmark Pynagent performance",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument(
        '--files', '-f', nargs='+', default=[],
        help='Files or directories to benchmark (default: Pynagent/rules/)',
    )
    parser.add_argument(
    ...
    if not args.files:
        Pynagent_rules = list(Path(__file__).parent.parent.glob("Pynagent/rules/*.py"))
        if Pynagent_rules:
            args.files = [str(Path(__file__).parent.parent / "Pynagent/rules")]
        else:
            print("No files specified and Pynagent/rules/ not found.")
            return 1
    ...
    if args.memory:
        ...
        print("=" * 60)
        memory_results = profile_memory(args.files or ['Pynagent/rules'])
...
def profile_memory(paths: List[str]) -> Dict[str, Any]:
    """Profile memory usage of Pynagent scanning."""
    import tracemalloc
    from Pynagent.core.engine import RuleEngine, clear_module_cache
...
def compare_with_competitors(files: List[Path]) -> List[Dict[str, Any]]:
    """Compare Pynagent with competitor tools."""
    import subprocess
    import shutil
    results = []

    # Pynagent baseline
    Pynagent_time = measure_Pynagent_time(files)
    results.append({
        'tool': 'Pynagent (Python)',
        'avg_time_ms': Pynagent_time,
        'files_per_sec': len(files) / (Pynagent_time / 1000) if Pynagent_time > 0 else 0,
    })
...
def measure_Pynagent_time(files: List[Path]) -> float:
    """Measure Pynagent scanning time."""
    import time
    clear_module_cache()
    _FALSE17 = (False,) * 17
    engine = _build_engine({}, None, *_FALSE17, debug_clean_mode='off')

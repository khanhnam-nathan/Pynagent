...
enum Commands {
    ...
    Upload {
        ...
        #[arg(long, default_value = "Pynagent-sast")]
        category: String,
    },
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    tracing_subscriber::registry()
        .with(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "Pynagent_rs=info".into()),
        )
    ...
    match args.command {
        ...
        Commands::Lsp { scan_on_save, scan_on_type, debounce_ms, severity, rules, stdio: _ } => {
            Pynagent_rs::run_server_with_args(Pynagent_rs::LspArgs {
                scan_on_save,
                scan_on_type,
                debounce_ms,
...
fn scan_paths(
    ...
    // Incremental scanning: load cache from .Pynagent-cache/
    let mut incremental_cache = if incremental {
        load_incremental_cache(Path::new(".Pynagent-cache"))
    } else {
        std::collections::HashMap::new()
    };
    ...
    if apply_fix {
        ...
        for (file_path, fix_list) in file_fixes_resolved {
            ...
            if !original_code.is_empty() {
                ...
                let _ = std::fs::copy(&Path::new(&file_path), Path::new(&format!("{}.Pynagent.bak", file_path)));
    ...
    if incremental {
        save_incremental_cache(Path::new(".Pynagent-cache"), &incremental_cache);
    }
...
fn generate_sarif_output(results: &[&ScanResult], ai_fix: bool) -> serde_json::Value {
    let mut builder = SarifBuilder::new(
        ...
        "https://github.com/Pynagent/Pynagent",
    );
...
<testsuite name="Pynagent" tests="{}" failures="{}" errors="0" skipped="0" timestamp="{}">
{}
</testsuite>"#,
        results.len(),
        results.len(),
...
<head>
    <meta charset="UTF-8">
    <title>Pynagent Security Report</title>
    <style>
...
<body>
    <h1>Pynagent Security Report</h1>
    <div class="summary">
        <h2>Summary</h2>
        <p>Total findings: "#,
    );
...
fn upload_results(
    ...
    match provider {
        ...
        UploadProvider::GitLab => {
            println!("GitLab upload: Create .gitlab/merge-request-Pynagent.yml for GitLab CI integration.");
        }
        UploadProvider::SonarQube => {
            println!("SonarQube integration: Use sonar-scanner with Pynagent SARIF output.");
        }
    }

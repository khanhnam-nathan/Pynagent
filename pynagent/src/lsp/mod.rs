//! Pynagent Language Server Protocol (LSP) Server
//!
...
pub struct PynagentLspServer {
    documents: Arc<Mutex<HashMap<String, Document>>>,
    config: LspConfig,
    debounce_tx: Option<std::sync::mpsc::Sender<DebounceCommand>>,
}

impl PynagentLspServer {
    pub fn new(config: LspConfig) -> Self {
        let _ = tracing_subscriber::fmt()
            .with_env_filter("Pynagent=warn")
            .try_init();
        tracing::info!("Pynagent LSP server starting on stdio...");
    ...
    fn handle_request(&self, req: JsonRpcRequest) -> Option<JsonRpcResponse> {
        match req.method.as_str() {
            "initialize" => {
                tracing::info!("Pynagent LSP: initialize request");
            ...
            "exit" => {
                tracing::info!("Pynagent LSP server exiting");
                std::process::exit(0);
            }
            "textDocument/hover" => {
    ...
    fn handle_notification(&mut self, method: &str, params: Value) {
        match method {
            ...
            "initialized" => {
                tracing::info!("Pynagent LSP: workspace initialized");
            }
            "workspace/didChangeConfiguration" => {
                if let Ok(settings) = serde_json::from_value::<serde_json::Value>(params.clone()) {
                    if let Some(Pynagent_config) = settings.get("Pynagent") {
                        if let Ok(_new_config) = serde_json::from_value::<LspConfig>(Pynagent_config.clone()) {
                            tracing::info!("Pynagent LSP: config updated");
                        }
                    }
                }
            }
    ...
    fn on_document_open(&mut self, params: DidOpenParams) {
        tracing::info!("Pynagent LSP: opening document {}", params.text_document.uri);
    ...
    fn on_hover(&self, req: JsonRpcRequest) -> Option<JsonRpcResponse> {
        ...
        for finding in findings {
            if finding.start <= cursor_byte && cursor_byte <= finding.end {
                let markdown = format!(
                    "**{}**  \n**Severity:** {}  \n**CWE:** {}  \n\n{}\n\n```\n{}\n```\n\n*Hover for details. Use `Pynagent explain {}` for more.*",
                    finding.rule_id,
                    finding.severity,
...
/// Run the Pynagent LSP server. Call this when `--lsp` flag is passed.
pub fn run_server() {
    let config = LspConfig::default();
    let mut server = PynagentLspServer::new(config);
    server.run();
}

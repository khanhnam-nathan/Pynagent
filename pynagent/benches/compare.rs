//! Head-to-head competitor comparison benchmarks for Pynagent.
//!
//! Compares Pynagent's Rust scanner against:
//! - Semgrep (via subprocess)
//! - Bandit (via subprocess)
//! - Pynagent's own Python implementation
//!
...
// --------------------------------------------------------------------------
// Pynagent (Rust) — measured via internal API
// --------------------------------------------------------------------------

fn bench_Pynagent_rust(c: &mut Criterion) {
    ...
    group.bench_function("Pynagent_rust", |b| {
        b.iter(|| {
...
fn bench_bandit(c: &mut Criterion) {
    ...
    group.bench_function("bandit", |b| {
        b.iter(|| {
            ...
            let test_file = temp_dir.join("Pynagent_benchmark_test.py");
...
// --------------------------------------------------------------------------
// Pynagent Python (for comparison)
// --------------------------------------------------------------------------

fn bench_Pynagent_python(c: &mut Criterion) {
    use Pynagent_rs::rules::security::all_security_rules;
    use Pynagent_rs::scanner::tree_sitter::parse;

    let mut group = c.benchmark_group("competitor_comparison");

    group.bench_function("Pynagent_python", |b| {
        b.iter(|| {
            let tree = parse(black_box(SAMPLE_CODE_PYTHON));
            if let Ok(tree) = tree {
                let rules = all_security_rules();
